...
> TFind 采用 **OAuth2 授权码模式** + **JWT 无状态令牌** 的双重认证机制，
> 网关作为 OAuth2 客户端，用户服务作为授权服务器。

### 认证流程

```
用户 → 网关(8080) → OAuth2 授权码流程 → 用户服务(8081) → 颁发 JWT
                        │
                        ▼
                  后续请求携带 JWT (Bearer Token)
                  网关 JwtAuthFilter 校验 → 转发到对应微服务
```
  ...
  security:
    oauth2:
      client:
        registration:
          tfind:
            client-id: tfind-client
            client-secret: tfind-secret
        ...
        provider:
          tfind:
            ...
            user-info-uri: http://localhost:8081/api/user/info
...
| 组件 | OAuth2 角色 | 说明 |
|---|---|---|
| **网关 `tfind-gateway`** | OAuth2 客户端 | 拦截请求，发起授权码流程，校验 JWT |
| **用户服务 `tfind-user-service`** | 授权服务器 + 资源服务器 | 处理登录、颁发 JWT、权限校验 |
| **厕所服务 `tfind-toilet-service`** | 资源服务器 | 通过 `JwtInterceptor` 校验 JWT |

### 安全组件

| 过滤器/拦截器 | 所在服务 | 功能 |
|---|---|---|
| [JwtAuthFilter]| 网关 | 校验 `Authorization: Bearer <token>`，解析 JWT |
| [JwtAuthenticationFilter]| 用户服务 | 提取请求头 Token，设置 SecurityContext |
| [JwtInterceptor] | 厕所服务 | 拦截器校验 JWT，解析 userId 和 role |
| [SecurityConfig (网关)]| 网关 | 放行白名单，其余需认证 |
| [SecurityConfig (用户服务)]| 用户服务 | 放开登录/注册，管理员接口需 `ROLE_ADMIN` |
...
普通用户（需登录）:
  GET/PUT /api/user/**       # 用户信息 / 修改密码

管理员（需 ROLE_ADMIN）:
  GET    /api/user/list      # 用户列表
  GET    /api/user/deleted   # 已删除用户
  DELETE /api/user/{id}      # 删除用户
  PUT    /api/user/{id}/restore    # 恢复用户
  POST   /api/user/{id}/admin     # 添加/移除管理员
  GET    /api/toilet/**      # 厕所管理（审核、删除等）
```
...
```yaml
jwt:
  secret: tfind-jwt-secret-key-2024-must-be-at-least-256-bits-long-for-hs256
  expiration: 86400000   # 24小时
```

- 签名算法：HMAC-SHA256 (HS256)
- 载荷字段：`userId`、`role`（ADMIN/USER）
- 微信小程序端：通过微信 `wx.login` 获取 `openid`，后端生成 JWT 返回
...
<a href="https://www.star-history.com/?repos=vinlucky%2FTFind&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=vinlucky/TFind&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=vinlucky/TFind&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=vinlucky/TFind&type=date&legend=top-left" />
 </picture>
</a>
